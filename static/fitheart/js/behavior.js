...
(function($) {
 ...
 $.asynchImageLoader = $.jail = {
  ...
  _isInTheScreen: function($ct, $img, optionOffset) {
   ...
   return (ct_top - optionOffset) <= (img_offset.top + img_height) && (ct_bottom + optionOffset) >= img_offset.top && (ct_left - optionOffset) <= (img_offset.left + img_width) && (ct_right + optionOffset) >= img_offset.left;
  },
...
qq.FileUploader = function(o) {
 ...
 qq.extend(this._options, {
  element: null,
  listElement: null,
  template: '<div class="qq-uploader">' + '<div class="qq-upload-drop-area"><span>Drop files here to upload</span></div>' + '<div class="btn qq-upload-button">Upload image</div>' + '<ul id="image_upload_list" class="qq-upload-list"></ul>' + '<div id="image_upload_error" class="help-block" style="color: #b94a48;"></div>' + "</div>",
...
qq.extend(qq.UploadHandlerForm.prototype, {
 ...
 _createS3Form: function(iframe, params, input, tid, sig, policy, success_redirect) {}
});
 ...
 if (resp.status == 0 || resp.responseText == "") {
  ...
  if (resp.status == 200 || resp.status == 406) {
   ...
   if (resp.responseText.trim().length > 0) {
    var data = jQuery.parseJSON(resp.responseText.trim());
   }
   if ("errors" in data) {
